...
int main() {
    ...
    vector<set<int> > pre(31,set<int>());
    pre[0].insert(0);
    for (int i=1; i<=30; i++) {
        for (int j=1; j<=i; j++) {
            int sure = j+(i+1-j);
            for (const int& u:pre[j-1]) {
                for (const int& v:pre[i-j]) {
                    pre[i].insert(u+v+sure);
                }
            }
        }
    }
    // for (int i=0; i<=30; i++) {
    //     for (int v:pre[i]) cout<<v<<' ';
    //     cout<<'\n';
    // }
    int t;
    cin>>t;
    while(t--) {
        int n,m;
        cin>>n>>m;
        set<int>::iterator u = pre[n].upper_bound(m);
        if (u==pre[n].begin()) {
            cout<<"-1\n";
        }
        else {
