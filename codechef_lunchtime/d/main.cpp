...
ll dfs(int node,int cur=0) {
    // cout<<cur<<' '<<' '<<val[node]<<' '<<s[cur]<<'\n';
    if (cur==l) return 1;
    if (dp[node][cur] != -1) return dp[node][cur];
    if (val[node] != s[cur]) return 0; 
    int ret = 0;
    for (int u:graph[node]) {
        ret = (ret + dfs(u,cur+1))%MOD;
    }
...
int main() {
    ...
    while(t--){
        ...
        if (ispalindrome(s)) (ans = (ans * pw2(2,MOD-2,MOD))%MOD);
        // if (ispalindrome(s)) ans/=2;
        cout<<ans<<'\n';
    }
    return 0;
}
