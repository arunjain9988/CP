...
ll ans(int st,int ed,int turn,vector<ll>& inputs,vector<vll>& dp) {
    if (st==ed) {
        return (turn==0)?inputs[st]:-inputs[st];
    }
    if (dp[st][ed] != -1) return dp[st][ed];
    if (turn==0) {
        return dp[st][ed] = max(inputs[st]+ans(st+1,ed,turn^1,inputs,dp),inputs[ed]+ans(st,ed-1,turn^1,inputs,dp));
    }
    else {
        return dp[st][ed] = min(-inputs[st]+ans(st+1,ed,turn^1,inputs,dp),-inputs[ed]+ans(st,ed-1,turn^1,inputs,dp));
    }
}
