...
long long noofways(int st,int d,bool strict) {
    if (st==k.size()) return (d==0);
    if (dp[st][d][strict] != -1) return dp[st][d][strict];
    int mx = (strict)?(k[st]-'0'):9;
    ll ans = 0;
    for (int i=0; i<=mx; i++) {
        ans = ans + noofways(st+1,(((d-i)%D)+D)%D,strict&(i==(k[st]-'0')));
        ans %=MOD;
    }
    return dp[st][d][strict] = ans;
}
...
int main() {
    ...
    cin>>k>>D;
    dp = vector<vector<vector<ll> > >(k.size(),vector<vector<ll> >(D,vector<ll>(2,-1)));
    cout<<(noofways(0,0,true)-1+MOD)%MOD;
    return 0;
}
