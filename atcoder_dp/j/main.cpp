...
double ans(int b,int c,int d,int n,vector<vector<vector<double> > >& dp) {
    // trace3(b,c,d);
    if (b==0 && c==0 && d==0) return 0;
    if (b<0 || c<0 || d<0) return 0;
    if (dp[b][c][d]  > -0.9 ) return dp[b][c][d];
    double ret = n+(b*(ans(b-1,c,d,n,dp)))+(c*(ans(b+1,c-1,d,n,dp)))+(d*(ans(b,c+1,d-1,n,dp)));
    return dp[b][c][d] = ret/(b+c+d);
}
...
int main() {
    ...
    cout<<ans(val[1],val[2],val[3],n,dp);
    return 0;
}
