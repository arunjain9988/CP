...
double pro(int coins,int headsreq,vector<double>& pr,vector<vector<double> >& dp) {
    if (headsreq <=0) return 1;
    if (coins == 0) return 0;
    if (dp[coins][headsreq] != -1) return dp[coins][headsreq];
    return dp[coins][headsreq] = (pro(coins-1,headsreq-1,pr,dp)*pr[coins] + pro(coins-1,headsreq,pr,dp)*(1-pr[coins]));
}
...
int main() {
    ...
    vector<vector<double> > dp(n+1,vector<double>(((n+1)/2)+1,-1));
    floop(i,1,n) {
        cin>>coins[i];
    }
    cout<<pro(n,(n+1)/2,coins,dp);
    return 0;
}
