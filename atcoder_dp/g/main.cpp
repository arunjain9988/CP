...
void dfs(int i,vector<int>& distances,vector<bool>& visited,vector<int> G[]) {
    visited[i] = true;
    distances[i] = 0;
    for (int u:G[i]) {
        if (!visited[u]) {
            dfs(u,distances,visited,G);
        }
        distances[i] = max(distances[u]+1,distances[i]);
    }
}
