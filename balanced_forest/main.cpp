...
int sz[50005];
int ans = 1<<30;
unordered_set<int,custom_hash> available;
...
void ansdfs(int node,int par=-1) {
    ...
    int remain = sz[1]-sz[node];
    if (remain%2==0 && sz[node]<=remain/2) {
        ans = min(ans,remain/2 - sz[node]);
    }
    int third = sz[1] - 2*sz[node];
    if (third <= (sz[1]-third) ){
        ans = min(ans,sz[node]-third);
    }
}
