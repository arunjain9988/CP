...
int main() {
    ...
    ll squaresA[n];
    ll squaresB[n];
    ll totalsqA = 0, totalsqB = 0;
    ll mulsq = 1;
    loop(i, n)
    {   
        squaresA[i] = (A[i] * A[i])%MOD;
        squaresB[i] = (B[i] * B[i])%MOD;
        totalsqA = (totalsqA + squaresA[i])%MOD;
        totalsqB = (totalsqB +  squaresB[i])%MOD;
        mulsq = (mulsq * ((A[i] * B[i])%MOD))%MOD;
    }
    ll ans = 0;
    loop(i,n) {
        ans = (ans + (squaresA[i] * (totalsqB - squaresB[i]))%MOD)%MOD;
    }
    loop(i,n) {
        ans = (ans - (2 * (mulsq * modinv(A[i] * B[i], MOD))))%MOD;
    }
