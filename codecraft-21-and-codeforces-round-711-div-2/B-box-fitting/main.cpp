...
map<int,int> occ;
void minimize_bachega(int a,int b) {
    auto it = occ.end();
    while(it!=occ.begin()) {
        it--;
        if (a>=it->first) {
            int hta_payenge = max(it->second,b);
...
int main() {
    ...
    while(t--) {
        ...
        while(occ.size()) {
            ii last = *occ.end();
            int kitne_ayenge = w/last.first;
            int min_chahiye_honge = last.second/kitne_ayenge;
            int extra_bachenge = last.second%kitne_ayenge;
            int wbachega = w - kitne_ayenge*last.first;
            minimize_bachega(wbachega,min_chahiye_honge);
            minimize_bachega(w-extra_bachenge,1);
        }
    }
    return 0;
}
