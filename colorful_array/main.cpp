...
int fl(int l,int r,int c,int* color,int next[]) {
    if (l>r) return l;
    // color[l] = c;
    // return next[l] = fl(next[l],r,c,color,next);
    if (color[l] == 0) {
        color[l] = c;
        return next[l] = fl(l+1,r,c,color,next);
    }
    else {
        return next[l] = fl(next[l],r,c,color,next);
    }
...
int main() {
    ...
    int next[n+2];
    iota(next,next+n+1,0);
    loop(i,q) {
        cin>>l[i]>>r[i]>>c[i];
    }
    for (int i=q-1; i>=0; i--) {
        fl(l[i],r[i],c[i],color,next);
    }
    floop(i,1,n) cout<<color[i]<<'\n';
    return 0;
}
