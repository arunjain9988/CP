...
int main() {
    ...
    for (; i<n*n; i++) {
        cin>>a;
        if (a&1) {
            cout<<2<<' '<<si-second.size()+1-max(si-n+1,0)<<' '<<second.size()+1+max(si-n+1,0)<<endl;
            cout<<flush;
            second.pb(2);
            if (second.size() == si+1) {
                second = vector<int>();
            ...
            cout<<1<<' '<<fi-first.size()+1-max(fi-n+1,0)<<' '<<first.size()+1+max(fi-n+1,0)<<endl;
            cout<<flush;
            first.pb(1);
            if (first.size() == fi+1) {
                first = vector<int>();
    ...
    while(i<n*n) {
        cin>>a;
        if (a!=3) {
            cout<<3<<' '<<fi-first.size()+1-max(fi-n+1,0)<<' '<<first.size()+1+max(fi-n+1,0)<<endl;
            cout<<flush;
            first.pb(3);
        }
        else {
            cout<<put<<' '<<fi-first.size()+1-max(fi-n+1,0)<<' '<<first.size()+1+max(fi-n+1,0)<<endl;
            cout<<flush;
            first.pb(put);
        }
        i++;
