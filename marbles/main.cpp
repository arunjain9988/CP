...
// useful constants
#define MOD 18446744073709551557L
...
__uint128_t fact(ll n,ll k) {
    if (n==k) return n;
    // cerr<<n<<'\n';
    return n*fact(n-1,k)%MOD;
}

void printuint(__uint128_t a) {
    if (a==0) return;
    int cur = a%10;
    printuint(a/10);
    cerr<<cur;
...
int main() {
    ...
    while(t--) {
        ...
        __uint128_t num = fact(n-1,n-k+1)%MOD;
        __uint128_t den = fact(k-1,1)%MOD;
        // printuint(num);
        // cerr<<'\n';
        // printuint(den);
        // cerr<<'\n';
